...
class UGATIT(object):
    def __init__(self, args):
        ...
        self.place = fluid.CUDAPlace(0) if self.device == 'cuda' else fluid.CPUPlace()
        self.start_iter = 1
    ...
    def build_model(self):
        """ DataLoader """
        self.trainA = reader_creater(os.path.join('data', self.dataset, 'trainA'), shuffle=True)
        self.trainB = reader_creater(os.path.join('data', self.dataset, 'trainB'), shuffle=True)
        if self.phase == 'train':
            self.testA = reader_creater(os.path.join('data', self.dataset, 'testA'))
            self.testB = reader_creater(os.path.join('data', self.dataset, 'testB'))
        if self.phase == 'test':
            self.testA = reader_creater(os.path.join('data', self.dataset, 'testA'),cycle=False)
            self.testB = reader_creater(os.path.join('data', self.dataset, 'testB'),cycle=False)
        ...
        self.G_optim = fluid.optimizer.AdamOptimizer(
            ...
            learning_rate= LinearDecay(lr=self.lr, iteration=self.iteration) if self.decay_flag else self.lr, 
            beta1=0.5, 
            beta2=0.999, 
    ...
    def test(self):
        ...
        with fluid.dygraph.guard(self.place): 
            ...
            if not len(model_list) == 0:
                ...
                print(" [*] Prepare to load the model in %d step ..."%(iter))
            ...
            self.genA2B.eval()
            self.genB2A.eval()
            with tqdm(total=100, desc='A2B') as pbar:
                for n, real_A in enumerate(self.testA_loader):
                    ...
                    cv2.imwrite(os.path.join(self.result_dir, self.dataset, 'test', 'A2B', 'A2B_%d.png' % (n + 1)), A2B * 255.0)
                    pbar.update()

            with tqdm(total=100, desc='B2A') as pbar:
                for n, real_B in enumerate(self.testB_loader):
                    ...
                    cv2.imwrite(os.path.join(self.result_dir, self.dataset, 'test', 'B2A', 'B2A_%d.png' % (n + 1)), B2A * 255.0)
                    pbar.update()
